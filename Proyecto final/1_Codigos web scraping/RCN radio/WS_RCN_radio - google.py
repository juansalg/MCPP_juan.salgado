...
# Keys usadas my_api_key1, my_api_key2, my_api_key3, my_api_key4, my_api_key5, my_api_key6,

keys = [my_api_key7, my_api_key8, my_api_key9, my_api_key10, my_api_key11]
...
"""
 Ya recopilados: 'Hurto bogotá','Seguridad ciudadana bogotá',
            ...
            'Acoso infantil bogotá','
"""

palabras = ['Seguridad bogotá','Homicidio bogotá','Golpiza bogotá','Linchamiento bogotá',
    ...
    for year in range(inicio, fin+1):
        ...
        try:
            ...
            for url in urls:                
                ...
                try:    
                    titulo = linksoup.find('div', attrs = {'class':'field--name-node-title'})
                ...
                try:
                    fecha = linksoup.find('div', attrs = {'class': 'field--name-field-pub-date'})
                    date = fecha.text
                    date = re.search(".* -", date).group()
                    date = date[:-1]
                except:
                    date = '**Especial**'
                    
                # Extracción del texto del articulo
                # Dos try-except porque se extrae la descripcion y el texto del articulo (1),
                # solo el texto del articulo (2) o se pone especial (3)
                try:
                    articulo_desc = linksoup.find('div', attrs = {'class':'field--name-dynamic-token-fieldnode-ds-summary'})
                    articulo_total = linksoup.find('div', attrs = {'class':'description-general'})
                    articulo_txt_desc = articulo_desc.find_all('p')
                    texto = ''
                    for i in range(len(articulo_txt_desc)):
                        texto = texto + " " + articulo_txt_desc[i].getText()
                    articulo_txt_tot = articulo_total.find_all('p')
                    for i in range(len(articulo_txt_tot)):
                        texto = texto + " " + articulo_txt_tot[i].getText()
                except:
                    try:
                        articulo_total = linksoup.find('div', attrs = {'class':'description-general'})
                        texto = ''
            ...
            a_buscar2 = 'site:www.rcnradio.com ' + a_buscar
...
test_df_tot.to_excel("RCN_radio_articulos_total_2.xlsx")
errores.to_excel("RCN_radio_errores_total_2.xlsx")
vacios.to_excel("RCN_radio_vacios_total_2.xlsx")
...
password = 'Cerac_2019'
message = """Subject: Finalizo WS de RCN radio
